window.addEventListener('load', ()=> {
    ...
    inputContainer.addEventListener("submit", (e) =>{
        ...
        // now adding all this shit in them main container

        leaderboard.appendChild(playerEl)

        // DELETE FUNCTIONALITY IT FUCKING WORKS YAY BUT FUCKING WHY? =))))

        deleteButton.addEventListener('click', () => {
            leaderboard.removeChild(playerEl)
        })
    ...
    function getTime() {
        ...
        const curentMonth = monthString[month].slice(0, 3)
        
        const curentTime = `${curentMonth} ${date} ${year} ${hour < 10 ?"0"+ hour : hour }:${minute < 10 ?"0"+ minute : minute}`
        return curentTime
    }
